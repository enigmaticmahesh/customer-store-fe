...
const handleNewTokensFetch = () => {
    ...
    const fetchNewTokens = async (oldRefreshToken: string): Promise<ApiResponse<AuthTokens>> => {
        if (activePromise) return activePromise
        const url = `${APP_ENV_CONFIG.API_BASE_URL}auth/get-tokens`
        activePromise = ky.post(url, {json: {refreshToken: oldRefreshToken}}).json<ApiResponse<AuthTokens>>()
			...
			async ({request, response, retryCount}) => {
				...
				if (response.status === 401 && retryCount === 0) {
                    try {
                        ...
                        useTokensStore.getState().updateStore({accessToken, refreshToken})
