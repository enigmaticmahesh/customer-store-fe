...
const AddToCartBtn = ({product}: {product: SortedProd}) => {
    const prodInCart = false
    const cartItems: {id: string, quantity: number}[] = []

    if (prodInCart) {
        const itemsList = cartItems.map( (item) => item.id === product._id && (
                <div key={item.id} className="flex flex-col w-11 h-22 items-center p-1 justify-between bg-primary text-primary-foreground ring-2 ring-white rounded-full">
                    <button
                        // onClick={() =>
                        //     updateItemQuantity(item.id, item.quantity - 1)
                        // }
                    >
                        <span className="text-xl cursor-pointer"> <Minus /> </span>
                    </button>
                    ...
                    <button
                        ...
                        // }
                    >
                        <span className="text-lg cursor-pointer"> <Plus /> </span>
                    </button>
                </div>
            ),
        )
